...
import agrfesta.k.cards.texasholdem.rules.gameplay.CallAction
import agrfesta.k.cards.texasholdem.rules.gameplay.FoldAction
...
class PlayerGameContextTest {
    ...
    fun testPlayerActionToString() {
        //TODO change it once done refactoring on Actions implementations
        assertThat( (alex does FoldAction()).toString() ).isEqualTo("Alex FOLD")
    }
    ...
    fun hasHistoryOnPreFlopOnlyReturnsANotEmptyListForPreFlopOnly() {
        val fold = FoldAction() //TODO change it once done refactoring on Actions implementations
        val context = buildingAPlayerGameContext()
